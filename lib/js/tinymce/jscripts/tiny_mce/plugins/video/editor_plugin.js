		...
		init : function(ed, url) {
			...
			ed.addCommand('mceVideo', function() {
				ed.windowManager.open({
					...
					height : 150 + parseInt(ed.getLang('video.delta_height', 0)),
					inline : 1
				}, {
			...
			function isVideoImg(node) {
				return node && node.nodeName === 'IMG' && ed.dom.hasClass(node, 'icon-video');
			};
			ed.onNodeChange.add(function(ed, cm, n) {
				cm.setActive('video', isVideoImg(n));
			});
		},
