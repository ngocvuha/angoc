<?php 
class QuanLyLop extends Module
{
	function QuanLyLop($row)
	{
		Module::Module($row);
		require_once 'db.php';
	...
	function export_cache()
	{
		if(User::can_view(false,ANY_CATEGORY))
		{
			$categogies = QuanLyLopDB::get_categories();
			$path = 'cache/tables/dslop.cache.php';
		...
		if(User::can_edit() and $move_structure_id=Url::get('move_structure_id') and $move_position=Url::get('move_position') and $structure_id=Url::get('structure_id') and $to_position=Url::get('to_position')){
			...
			if(IDStructure::parent($move_structure_id)==IDStructure::parent($structure_id)){
				...
				for($i=0;$i<$n;$i++){
					...
					//si_move_position('tblLopNienChe');
				}
			}else{
	...
	function change_parent(){
		if(User::can_edit() and $move_structure_id=Url::get('move_structure_id') and $droppable_structure_id=Url::get('droppable_structure_id')){
			require_once 'packages/core/includes/system/si_database.php';
			si_move('tblLopNienChe',$move_structure_id, $droppable_structure_id);
		}
		Url::redirect_current();
	}
	...
	function add_cmd()
	{
		if(User::can_add(false,ANY_CATEGORY))
		{
			...
			$this->add_form(new EditQuanLyLopForm());
		}
		else
		{
			Url::redirect_current();
	...
	function delete_cmd()
	{
		if($items=Url::get('selected_ids') and is_array($items) and sizeof($items)>0 and User::can_delete())
		{
			if($ids=implode(',',$items)){
				DB::delete('tblLopNienChe','id IN ('.$ids.')');
			}
	...
	function edit_cmd()
	{
		if(Url::iget('id') and $category=DB::fetch('select id,structure_id from tblLopNienChe where id='.Url::iget('id')) and User::can_edit(false,$category['structure_id']))
		{
			require_once 'forms/edit.php';
			$this->add_form(new EditQuanLyLopForm());
		}
	...
	function list_cmd()
	{
		if(User::can_view(false,ANY_CATEGORY))
		{
			...
			$this->add_form(new ListQuanLyLopForm());
		}	
		else
		{
			Url::access_denied();
	...
	function move_cmd()
	{
		if(User::can_edit(false,ANY_CATEGORY) and Url::check('id') and $category=DB::exists_id('tblLopNienChe',$_REQUEST['id']))
		{
