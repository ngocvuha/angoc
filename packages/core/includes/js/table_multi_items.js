...
function init_fields(index)
{
	for(i in table_fields)
	{
		if(i && $(i+'_'+index))
		{
			...
			{
				var action = ' onfocus="$(\'suggest_box\').style.display=\'none\';this.last_value=this.value;this.parentNode.className=\'selected_input\';start_suggest = false;last_suggest = false;next_suggest = false;" onblur="this.parentNode.className=\'normal_input\';"'+define_field_actions[i].replace(/#index#/g,index);
			}
			else
			{
				var action = ' onfocus="$(\'suggest_box\').style.display=\'none\';this.last_value=this.value;this.parentNode.className=\'selected_input\';start_suggest = false;last_suggest = false;next_suggest = false;" onblur="this.parentNode.className=\'normal_input\';'+((table_fields[i]=='suggest')?'$(\'suggest_box\').style.display=\'none\';':'')+'"';
			}
			$(i+'_'+index).className = 'normal_input';
			switch(table_fields[i])
			{
			case 'select':
			if(define_select_fields[i])
			{
				$(i+'_'+index).innerHTML='<select name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input" '+action+'>'+define_select_fields[i]+'</select>';
			}
			else
			{
				$(i+'_'+index).innerHTML='<input type="text" name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input" '+action+'>';
			}
			break;
			case 'suggest':
			if(define_suggest_fields[i])
			{
				$(i+'_'+index).innerHTML='<input name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input" '+action+' onkeyup="update_suggest_box(this,\''+i+'\');">';
			...
			break;
			case 'currency':
				$(i+'_'+index).innerHTML='<input type="text" name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input"  onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=44 && event.keyCode!=36 && event.keyCode!=13 && event.keyCode!=45)event.returnValue=false;" '+action+'>';
				break;
			case 'int':
				$(i+'_'+index).innerHTML='<input type="text" name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input"  onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=13)event.returnValue=false;" '+action+'>';
				break;
			case 'float':
				$(i+'_'+index).innerHTML='<input type="text" name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input"  onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=44 && event.keyCode!=13)event.returnValue=false;" '+action+'>';
				break;
			case 'checkbox':
...
function add_row(init_values,dont_move_focus)
{
	...
	td.innerHTML = '<a  onclick="delete_row('+new_index+');"><img tabIndex="-1" src="skins/default/images/buttons/delete.gif"></a>';
	var field_index = 0;
	for(var i in table_fields)
	{
		if(i)
		{
			...
			if(typeof(define_field_actions[i])!='undefined' && define_field_actions[i])
			{
				var action = ' onfocus="$(\'suggest_box\').style.display=\'none\';this.last_value=this.value;this.parentNode.className=\'selected_input\';start_suggest = false;last_suggest = false;next_suggest = false;" onblur="this.parentNode.className=\'normal_input\';"'+define_field_actions[i].replace(/#index#/g,'new_row_'+new_index);
			}
			else
			{
				var action = ' onfocus="$(\'suggest_box\').style.display=\'none\';this.last_value=this.value;this.parentNode.className=\'selected_input\';start_suggest = false;last_suggest = false;next_suggest = false;" onblur="this.parentNode.className=\'normal_input\';'+((table_fields[i]=='suggest')?'$(\'suggest_box\').style.display=\'none\';':'')+'"';
			...
			case 'select':
				if(define_select_fields[i])
				{
					td.innerHTML = '<div class="normal_input"><select  name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input"'+action+'>'+define_select_fields[i]+'</select></div>';
				}
				else
				{
					td.innerHTML = '<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input"'+action+'></div>';
				}
			break;
			case 'suggest':
			if(define_suggest_fields[i])
			{
				td.innerHTML = '<div class="normal_input"><input name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" '+action+' onkeyup="update_suggest_box(this,\''+i+'\');"></div>';
			...
			{
				$(i+'_'+new_index).innerHTML='<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" '+action+'></div>';
			}
			break;
			case 'currency':
				td.innerHTML = '<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=44 && event.keyCode!=36 && event.keyCode!=13 && event.keyCode!=45)event.returnValue=false;"'+action+'></div>';
				break;
			case 'int':
				td.innerHTML = '<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=13)event.returnValue=false;"'+action+'></div>';
				break;
			case 'float':
				td.innerHTML = '<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" onkeypress="if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 && event.keyCode!=44 && event.keyCode!=13)event.returnValue=false;"'+action+'></div>';
				break;
			case 'checkbox':
				td.innerHTML = '<div class="normal_input"><input type="checkbox" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input" value="1"'+action+'></div>';
				break;
			default:
				td.innerHTML = '<div class="normal_input"><input type="text" name="records[new_row_'+new_index+']['+i+']" id="records_new_row_'+new_index+'_'+i+'" class="normal_input"'+action+'></div>';
				break;
...
function init_search_row()
{
	var tr = $('main_table').insertRow(1);
	var td = tr.insertCell(-1);
	td.innerHTML = '<input type="image" src="skins/default/images/buttons/search.gif"/>';
	for(var i in table_fields)
	{
		if(i)
		{
			...
			case 'select':
				if(define_select_fields[i])
				{
					td.innerHTML = '<select  name="search_by_'+i+'" id="search_by_'+i+'" class="normal_input" onchange="this.form.submit();"><option value=""></option>'+define_select_fields[i]+'</select>';
				}
				else
				{
					td.innerHTML = '<input  type="text" name="search_by_'+i+'" id="search_by_'+i+'" class="normal_input">';
				}
				break;
			case 'checkbox':
				td.innerHTML = '<input  type="checkbox" name="search_by_'+i+'" id="search_by_'+i+'" class="normal_input" value="1">';
				break;
			default:
				td.innerHTML = '<input  type="text" name="search_by_'+i+'" id="search_by_'+i+'" class="normal_input">';
				break;
...
function change_to_select(input,i,index)
{
	...
	div.innerHTML = '<select name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input"'+action+'>'+define_select_fields[i]+'</select>'
...
function change_to_input(input,i,index)
{
	...
	div = input.parentNode;
	var value = $('records_'+index+'_'+i).value;
	div.innerHTML = '<input type="text" name="records['+index+']['+i+']" id="records_'+index+'_'+i+'" class="normal_input"'+action+'>';
	$('records_'+index+'_'+i).value = value;
}
...
function update_suggest_box(input,index)
{
	...
	{
		var pos = jQuery('#'+input).position();
		var bound_pos = jQuery('#'+$('center_region')).position();
		$('suggest_box').style.left = pos.top-bound_pos.top;
		$('suggest_box').style.top = pos.left-bound_pos.left+getElementHeight(input.id);
