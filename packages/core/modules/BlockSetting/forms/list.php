					...
					if($file_name and isset($all_items[$setting_id]) and ($all_items[$setting_id]['type']=='FILE' or $all_items[$setting_id]['type']=='IMAGE'))
					{
						...
						if($can_edit)
						{
							...
							else
							{
								...
								if(!is_dir('upload/config/block_settings'))
								{
									mkdir('upload/config/block_settings');
								}
								$new_file_name = 'upload/config/block_settings/'.str_replace('#','_',$block['id']).'_'.time().'_'.$_FILES['items']['name'][$setting_id];
		...
		if($this->get_items($block, $all_items, $item_values))
		{
			...
			foreach($all_items as $setting_id=>$item)
			{
				...
				if($can_view)
				{
					...
					$groups[$item['group_name']]['code']=convert_utf8_to_url_rewrite($item['group_name']);
				}
			}
