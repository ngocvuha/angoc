...
{
	DB::query('select * from '.$table.' where structure_id='.IDStructure::parent(($structure_id>=ID_ROOT)?$structure_id:DB::structure_id($table,$structure_id)).$extra);
	$row = DB::fetch();
	return $row;
}
function si_parent_id($table,$structure_id, $extra='')
{
	...
	$row = si_parent($table,$structure_id, $extra);
	return $row['id'];
}
...
function si_next_child($table,$structure_id,$extra_cond = '')
{
	...
	if($level>8) $level=8;
	$child_offset = bcadd(bcpow(ID_BASE, ID_MAX_LEVEL-($level+1)),0,0);
	...
	$rows = DB::fetch_all();
	$new_id = bcadd($structure_id,$child_offset,0);
	for($i=1;$i<ID_BASE;$i++)
	{
		$found = false;
		if($rows)
		...
		$new_id = bcadd($new_id,$child_offset,0);
	}
	return $new_id;
}
...
function si_child($table,$structure_id,$extra_cond = '', $database=false)
{
	...
	$new_id = bcadd($structure_id,$child_offset,0);
	if($rows){
		$arr_structure_id = array();
		foreach($rows as $row){
			$arr_structure_id[] = $row['structure_id'];
		}
		$max = max($arr_structure_id);
		$child_max_offset = substr(strval($max),-2,2);
		if($child_max_offset < 99){
			return bcadd($max,$child_offset,0);
		}
	}
	return $new_id;
}
