<div class="middle">
	<div class="content-header clrfix">
        ...
        <div class="fr">
        <?php if((User::can_add())){?><button id="add" class="red-button">Thêm mới</button>
		<button id="excel" class="blue-button" onclick="window.location='<?php echo Url::build_current(array('cmd'=>'import'));?>'">Import From Excel</button>
		<?php } ?>
        ...
        <form name="ListManageCustomerForm" method="post">
            ...
            <table width="100%" cellpadding="2" cellspacing="0" border="1" style="border-collapse:collapse" bordercolor="#CCCCCC">
                <tr class="ht">
                    ...
                    <td nowrap><a href="<?php echo String::order_by('account.MaSV');?>" class="orderby<?php echo String::order_by_active('account.MaSV');?>" title="<?php echo Portal::language('sort');?>">Mã SV</a></td>
					<td nowrap><a href="<?php echo String::order_by('account.id');?>" class="orderby<?php echo String::order_by_active('account.id');?>" title="<?php echo Portal::language('sort');?>">Họ Tên</a></td>
                    ...
                    <td nowrap width="200"><a href="<?php echo String::order_by('customer_group.name_'.Portal::get_setting('default_language'));?>" class="orderby<?php echo String::order_by_active('customer_group.name_'.Portal::get_setting('default_language'));?>" title="<?php echo Portal::language('sort');?>"><?php echo Portal::language('customer_group');?></a></td>
                ...
                <tr class="ht">
                    ...
                    <td><input  name="search_MaSV" id="search_MaSV" class="search-field" autofocus style="width:60px" / type ="text" value="<?php echo String::html_normalize(URL::get('search_MaSV'));?>"></td>
                    ...
                    <td><select  name="tblLopNienChe" id="tblLopNienChe" class="search-field"><?php
					if(isset($this->map['tblLopNienChe_list']))
					{
						foreach($this->map['tblLopNienChe_list'] as $key=>$value)
						{
					...
					?></select><script type="text/javascript">$('tblLopNienChe').value = "<?php echo addslashes(URL::get('tblLopNienChe',isset($this->map['tblLopNienChe'])?$this->map['tblLopNienChe']:''));?>";</script></td>
                ...
                <tr valign="middle" <?php Draw::hover('#E2F1DF');?> style="<?php if($i%2){echo 'background-color:#F9F9F9';} $i++;?>" id="ListManageCustomerForm_tr_<?php echo $this->map['items']['current']['id'];?>">
                    ...
                    <td nowrap><?php echo $this->map['items']['current']['MaSV'];?></td>
					<td nowrap><?php echo urldecode($this->map['items']['current']['HoDem'].' '.$this->map['items']['current']['Ten']);?></td>
                    <td nowrap><?php echo $this->map['items']['current']['email'];?></td>
                    <td nowrap><?php echo $this->map['items']['current']['LopNienChe'];?></td>
