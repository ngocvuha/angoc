<?php
class ListQuanLyGiamThiForm extends Form
{
	function ListQuanLyGiamThiForm()
	{
		Form::Form('ListQuanLyGiamThiForm');
	}
	function draw()
	{
		$this->map=array();
		...
		$this->map['total'] = QuanLyGiamThiDB::get_total_item($cond);
		...
		$this->map['items'] = QuanLyGiamThiDB::get_items($cond,String::order_by_sql($order_field,$order_default),$item_per_page);
	...
	{
		$privilege_id = 3;
		$cond = "account.type='USER' and account.id!='admin' and account_privilege.privilege_id = ".$privilege_id;
		// account
