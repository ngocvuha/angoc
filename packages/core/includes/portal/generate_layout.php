...
class GenerateLayout
{
	...
	function generate_text($text)
	{
			...
			if($pos2 = strpos($text,'"',$pos+15))
			{
				...
				if($items = $this->get_text_variable($var.'_list'))
				{
					...
					$result.=preg_replace(
						...
						substr($text, $p+1,  $p2-$p-1)).'</select><script type="text/javascript">$(\''.$var.'\').value = "<?php echo addslashes(URL::get(\''.$var.'\',isset($this->map[\''.$var.'\'])?$this->map[\''.$var.'\']:\'\'));?>";</script>';
		...
		while(($pos=strpos($text,'<!--IF:'))!==false)
		{
			...
			{
				$result .= substr($text, 0,  $pos);
				$result .= '<?php if('.substr($text, $pos2,  $pos4-$pos2).'){?>';
				$result .= $this->generate_text(strtr(substr($text, $pos4+3,  $pos3-$pos4-3),array('<!--ELSE-->'=>' <?php }else{ ?>'))).'<?php } ?>';
		...
		while(($pos=strpos($text,'{{'))!==false)
		{
			if($pos2 = strpos($text,'}}',$pos+2))
			{
				$name = substr($text, $pos+2,  $pos2-$pos-2);
				$result .= substr($text, 0,  $pos).'<?php echo '.$name.';?>';
				$text = substr($text, $pos2+2,  strlen($text)-$pos2-2);
			}
			else
			{
				$result .= substr($text, 0,  $pos+2);
				$text = substr($text, $pos+2,  strlen($text)-$pos-2);
			}
		}
		$text = $result.$text;
		$result = '';
		while(($pos=strpos($text,'<.'))!==false)
		{
			...
			{
				$name = substr($text, $pos+2,  $pos2-$pos-2);
				$result .= substr($text, 0,  $pos).'<?php '.$name.'?>';
				$text = substr($text, $pos2+2,  strlen($text)-$pos2-2);
			}
			else
			{
